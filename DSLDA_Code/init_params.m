...
if(option>=4)
    ...
    if(option==7) % DSLDA-OSST
        model.epsilon = (1-1e-50);
    elseif(option==8) % DSLDA-NSLT
        model.epsilon = 1e-50;
    else
    ...
    if (option==5 || option==8)     %% DSLDA-NSLT
    ...
    model.alpha   = randind*ones(1,model.K)/model.K;
end

model.gamma    = randind*ones(model.N,model.K)*(model.N/model.K);   % dimension N*K   %% have to change
