...
model* train(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type == MCSVM_CS)
	{
		...
		model_->alpha=Solver.Solve(model_->w);
                //printf("cscsdcdc %f", model_->alpha[3]);
                //getchar(); 
	}
	else
	{
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, int *target)
{
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		struct model *submodel = train(&subprob,param);
                //printf("dsdfv %f", submodel->alpha[3]); 
		for(j=begin;j<end;j++)
...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	/*FILE *fp1 = fopen("/home/ayan/Dropbox/DSLDA_SDM/onlineHDP/alphaval.txt","w");
        if (ferror(fp1) != 0)
	...
	for(i=0; i<model_->nr_trpts; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
			fprintf(fp1, "%.16g ", model_->alpha[i*nr_w+j]);
		fprintf(fp1, "\n");
	}
        fclose(fp1);*/
...
void get_w(const struct model *model_)
{
 int i, w_size, nr_w;
 printf("hey here1!");

 w_size = model_->nr_feature;
 nr_w=model_->nr_class;  // number of classes
 
 printf("%d %d", w_size, nr_w);
 FILE *fp = fopen("wval.txt","w");
 /*for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
...
void get_alpha(const model *model_)
{
 int i, n_size, nr_w;

 n_size = model_->nr_trpts;
 nr_w = model_->nr_class;

 printf("hey here!");

 FILE *fp = fopen("alphaval.txt","w+");
 for(i=0; i<n_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
