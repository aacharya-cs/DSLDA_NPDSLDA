...
xmalloc (int n)
{
  void *p = malloc (n);
  if (!p)
    larank_error ("Function malloc() has returned zero\n");
  return p;
}
...
xrealloc (void *ptr, int n)
{
  if (!ptr)
    ...
    ptr = realloc (ptr, n);
  if (!ptr)
    larank_error ("Function realloc has returned zero\n");
  return ptr;
}
...
xextend (larank_kcache_t * self, int k, int nlen)
{
  ...
  if (nlen > olen)
      ...
      if (olen > 0)
	{
      ...
      self->maxrowlen = jmax (self->maxrowlen, nlen);
    }
}
