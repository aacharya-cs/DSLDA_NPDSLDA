function [] = OLGammaUpdate(model, testdata)
...
model.gamma = term1+term2;
    ...
    ind1 = find(testdata.annotations==0); %% since latent topics are appended as extra columns; does not make a difference in indexing
    ...
    if(model.option==5)    %% DSLDA-NSLT
        ...
        missingdata = find(testdata.classlabels==0); %% get the missing data
        ind24(missingdata,:) = [];
        ind2(ind24(:)) = 0;
